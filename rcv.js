 ...
 * @return {Object} Object mapping candidate/bucket names to the number of ballots that were
 *   redistributed to that candidate/bucket.
 */
...
function getSources( buckets ) {
    let sources = {};
    for ( let candidate in buckets ) {
        sources[candidate] = {};
        for ( let ballot of buckets[candidate] ) {
            let source = ballot.initialBucket;
            sources[candidate][source] = sources[candidate][source] || 0;
...
function cloneBuckets( buckets ) {
    let result = {};
    for ( let candidate in buckets ) {
        result[candidate] = buckets[candidate].slice();
    }
    return result;
}
 ...
 * @param {Object[]} ballots Ballots in the format generated by parse.js
 * @param {boolean} [includeBuckets] Set to true to include verbose per-ballot data
 * @return {Object[]} Object describing each round
 */
function runContest( ballots, includeBuckets ) {
    ...
    while ( candidates.length - eliminated.length > 2 ) {
        if ( includeBuckets ) {
            clone = cloneBuckets( buckets );
        }
        let roundCounts = bucketCounts( buckets ),
            sources = getSources( buckets ),
            loser = losingCandidate( buckets, eliminated ),
            redistributions = redistribute( buckets, loser, eliminated );
        eliminated.push( loser );
        rounds.push( { roundCounts, sources, loser, redistributions } );
        if ( includeBuckets ) {
            rounds[rounds.length - 1].buckets = clone;
        }
    }
    rounds.push( {
        roundCounts: bucketCounts( buckets ),
        sources: getSources( buckets ),
        winner: winningCandidate( buckets )
    } );
    if ( includeBuckets ) {
        rounds[rounds.length - 1].buckets = buckets;
    }
    return rounds;
}
